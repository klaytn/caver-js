...
const humanreadableAddressPrefix = chance
    .last()
    .replace(' ', '')
    .slice(0, 12)
console.log('humanreadableAddressPrefix is: ', humanreadableAddressPrefix)

const specificPrivateKey =
    process.env.privateKey && String(process.env.privateKey).indexOf('0x') === -1 ? '0x' + process.env.privateKey : process.env.privateKey
            ...
            .on('receipt', async receipt => {
                ...
                caver.klay
                    ...
                    .on('receipt', async () => {
                        ...
                        const expectedAccountBalance = new BigNumber(reservoirBeforeBalance)
    ...
    `, done => {
        ...
        caver.klay
            ...
            .on('receipt', async receipt => {
                ...
                caver.klay
                    .sendTransaction(feePayerTransaction)
                    .on('transactionHash', console.log)
                    .on('receipt', async receipt => {
            ...
            .on('receipt', async receipt => {
                ...
                caver.klay
                    ...
                    .on('receipt', async receipt => {
                        ...
                        caver.klay.sendTransaction(feePayerTransaction).on('receipt', async receipt => {
                            const gasPrice = await caver.klay.getGasPrice()
                            const usedGasPrice = new BigNumber(gasPrice).multipliedBy(receipt.gasUsed)
            ...
            .on('receipt', async receipt => {
                ...
                caver.klay
                    ...
                    .on('receipt', async receipt => {
                        ...
                        caver.klay.sendTransaction(feePayerTransaction).on('receipt', async receipt => {
                            const gasPrice = await caver.klay.getGasPrice()
                            const usedGasPrice = new BigNumber(gasPrice)
                                .multipliedBy(receipt.gasUsed)
                                .multipliedBy(feeRatio / 100)
                                .toString(10)
                            const _transaction = await caver.klay.getTransaction(receipt.transactionHash || receipt.txHash)
