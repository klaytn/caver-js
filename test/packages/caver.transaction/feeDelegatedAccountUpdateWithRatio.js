...
describe('TxTypeFeeDelegatedAccountUpdateWithRatio', () => {
    ...
    beforeEach(() => {
        txsByAccountKeys = []
        txsByAccountKeys.push(new caver.transaction.feeDelegatedAccountUpdateWithRatio(txObjWithLegacy))
        txsByAccountKeys.push(new caver.transaction.feeDelegatedAccountUpdateWithRatio(txObjWithPublic))
        txsByAccountKeys.push(new caver.transaction.feeDelegatedAccountUpdateWithRatio(txObjWithFail))
        txsByAccountKeys.push(new caver.transaction.feeDelegatedAccountUpdateWithRatio(txObjWithMultiSig))
        txsByAccountKeys.push(new caver.transaction.feeDelegatedAccountUpdateWithRatio(txObjWithRoleBased))
    ...
    context('feeDelegatedAccountUpdateWithRatio.signWithKey', () => {
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-168: input: keyring, custom hasher. should throw error.', async () => {
    ...
    context('feeDelegatedAccountUpdateWithRatio.signFeePayerWithKey', () => {
        const txHash = '0xe9a11d9ef95fb437f75d07ce768d43e74f158dd54b106e7d3746ce29d545b550'
        ...
        beforeEach(() => {
            ...
            createFromPrivateKeySpy = sandbox.spy(Keyring, 'createFromPrivateKey')
            feePayerSignWithKeySpy = sandbox.spy(feePayer, 'signWithKey')
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-172: input: keyring. If feePayer is not defined, should be set with keyring address.', async () => {
            for (let i = 0; i < txsByAccountKeys.length; i++) {
                const tx = txsByAccountKeys[i]
                tx.feePayer = '0x'
                await tx.signFeePayerWithKey(feePayer)

                expect(tx.feePayer.toLowerCase()).to.equal(feePayer.address.toLowerCase())
                checkFunctionCall(i)
                checkFeePayerSignature(tx)
                expect(feePayerSignWithKeySpy).to.have.been.calledWith(txHash, '0x7e3', 2, 0)
            }
            expect(createFromPrivateKeySpy).not.to.have.been.called
        }).timeout(200000)
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-174: input: private key string. should sign transaction.', async () => {
            ...
            for (let i = 0; i < txsByAccountKeys.length; i++) {
                const tx = txsByAccountKeys[i]
                await tx.signFeePayerWithKey(feePayer.keys[0][0].privateKey)
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-175: input: KlaytnWalletKey. should sign transaction.', async () => {
            ...
            for (let i = 0; i < txsByAccountKeys.length; i++) {
                const tx = txsByAccountKeys[i]
                await tx.signFeePayerWithKey(feePayer.getKlaytnWalletKey())
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-177: input: keyring, custom hasher. should throw error.', async () => {
            const hashForCustomHasher = '0x9e4b4835f6ea5ce55bd1037fe92040dd070af6154aefc30d32c65364a1123cae'
            const customHasher = () => hashForCustomHasher
    ...
    context('feeDelegatedAccountUpdateWithRatio.signWithKeys', () => {
        const txHash = '0xe9a11d9ef95fb437f75d07ce768d43e74f158dd54b106e7d3746ce29d545b550'
    ...
    context('feeDelegatedAccountUpdateWithRatio.signFeePayerWithKeys', () => {
        const txHash = '0xe9a11d9ef95fb437f75d07ce768d43e74f158dd54b106e7d3746ce29d545b550'
        ...
        beforeEach(() => {
            ...
            createFromPrivateKeySpy = sandbox.spy(Keyring, 'createFromPrivateKey')
            feePayerSignWithKeysSpy = sandbox.spy(feePayer, 'signWithKeys')
            hasherSpy = sandbox.stub(TransactionHasher, 'getHashForFeePayerSignature')
            hasherSpy.returns(txHash)
        })
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-187: input: keyring. If feePayer is not defined, should be set with keyring address.', async () => {
            for (let i = 0; i < txsByAccountKeys.length; i++) {
                const tx = txsByAccountKeys[i]
                tx.feePayer = '0x'
                await tx.signFeePayerWithKeys(feePayer)
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-189: input: private key string. should sign transaction.', async () => {
            ...
            for (let i = 0; i < txsByAccountKeys.length; i++) {
                const tx = txsByAccountKeys[i]
                await tx.signFeePayerWithKeys(feePayer.keys[0][0].privateKey)
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-190: input: KlaytnWalletKey. should sign transaction.', async () => {
            ...
            for (let i = 0; i < txsByAccountKeys.length; i++) {
                const tx = txsByAccountKeys[i]
                await tx.signFeePayerWithKeys(feePayer.getKlaytnWalletKey())
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-191: input: keyring, custom hasher. should use custom hasher when sign transaction.', async () => {
            ...
            for (let i = 0; i < txsByAccountKeys.length; i++) {
                ...
                await tx.signFeePayerWithKeys(roleBasedKeyring, customHasher)
    ...
    context('feeDelegatedAccountUpdateWithRatio.combineSignatures', () => {
        let combinedTarget
        beforeEach(() => {
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-202: combineSignatures combines single signature and sets signatures in transaction', () => {
            const tx = new caver.transaction.feeDelegatedAccountUpdateWithRatio(combinedTarget)
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-203: combineSignatures combines multiple signatures and sets signatures in transaction', () => {
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-204: combineSignatures combines single feePayerSignature and sets feePayerSignatures in transaction', () => {
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-205: combineSignatures combines multiple feePayerSignatures and sets feePayerSignatures in transaction', () => {
            combinedTarget.feePayer = '0xa317526534d82b902e86c960e037ede7b83af824'
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-206: combineSignatures combines multiple signatures and feePayerSignatures', () => {
            let tx = new caver.transaction.feeDelegatedAccountUpdateWithRatio(combinedTarget)
            ...
            tx = new caver.transaction.feeDelegatedAccountUpdateWithRatio(combinedTarget)
        ...
        it('CAVERJS-UNIT-TRANSACTIONFDR-207: If decode transaction has different values, combineSignatures should throw error', () => {
            const tx = new caver.transaction.feeDelegatedAccountUpdateWithRatio(combinedTarget)
            tx.nonce = 1234
