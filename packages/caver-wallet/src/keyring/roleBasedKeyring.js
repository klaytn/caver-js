...
class RoleBasedKeyring extends AbstractKeyring {
    ...
    signWithKey(transactionHash, chainId, role, index = 0) {
        ...
        return keys[optionalParams.index].sign(transactionHash, chainId)
    }
    ...
    signWithKeys(transactionHash, chainId, role) {
        validateForSigning(transactionHash, chainId)
        if (role === undefined) role = KEY_ROLE.roleTransactionKey

        const signatures = []

        for (let i = 0; i < this.getKeyByRole(role).length; i++) {
